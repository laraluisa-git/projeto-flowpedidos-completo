...
export default function Inventory() {
  ...
  function saveProduct() {
    ...
    DB.write((d, audit) => {
      if (!editingId) {
        const customId = String(form.productId || "").trim();
        if (customId && d.products.some((p) => p.id === customId)) {
          alert("Já existe um produto com este ID.");
          return d;
        }
        const p = { id: customId || DB.uid("p"), name: form.name, category: form.category, stockQty: form.stockQty, minStockQty: form.minStockQty, unitPrice: form.unitPrice, isActive: form.isActive };
        d.products.unshift(p);
        audit("CREATE", "product", p.id, `Produto criado: ${p.name}`);
        return d;
      }
      const idx = d.products.findIndex((x) => x.id === editingId);
      if (idx === -1) return d;
      const updated = { ...d.products[idx], ...form };
      d.products[idx] = updated;
      audit("UPDATE", "product", updated.id, `Produto atualizado: ${updated.name}`);
      return d;
    });
  ...
  function deleteProduct(id) {
    ...
    DB.write((d, audit) => {
      ...
      audit("DELETE", "product", id, `Produto excluído: ${p.name}`);
      return d;
    });
    setRefresh((n) => n + 1);
  }
